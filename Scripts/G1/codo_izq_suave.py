...
class Custom:
    ...
    def Start(self):
        ...
        while self.first_update_low_state == False:
            time.sleep(1)
            print("Aun esperando low_state")

        #if self.first_update_low_state == True:
        print("LowState recibido. Iniciando")
    ...
    def LowCmdWrite(self):
        ...
        if self.start_pos is None:
            self.start_pos = [self.low_state.motor_state[joint].q for joint in self.arm_joints]
        print("Starte pos", self.start_pos)

        if self.counter_ %60 == 0:
            print(f"Control activo | time = {self.time_:.2f}")
        self.counter_ += 1
        ...
        if self.time_ < self.duration_ :
          ...
          for i,joint in enumerate(self.arm_joints):
            ...
            target = self.start_pos[i] + self.target_pos[i]
            self.low_cmd.motor_cmd[joint].q = ratio * target + (1.0 - ratio) * self.start_pos[i]
          ...
          for i,joint in enumerate(self.arm_joints):
              ...
              if self.counter_ %50 == 0:
                  print(f"joint={joint} target={self.low_cmd.motor_cmd[joint].q:.3f} actual={self.low_state.motor_state[joint].q:.3f}")
          ...
          for i,joint in enumerate(self.arm_joints):
              ...
              self.low_cmd.motor_cmd[joint].tau = 0.
              target = self.start_pos[i] + self.target 
